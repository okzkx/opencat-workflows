...
- 若执行过程中发现不明来源变更或新 TODO 项，不能暂停；优先交由后续 OpenCat 链路将异常变更作为独立 Git 提交收口，再继续原有任务
- 异常变更提交时应尽量缩小范围，避免混入当前任务的未完成改动
...
- 发现不明来源变更或新 TODO 项时，不暂停询问；记录归属并推动后续 cleanup/task 链路以独立提交方式收口
- 每次安装或引导修复完成后，都要立即验证
