---
name: opencat-task
description: OpenSpec 单任务分阶段执行器。**必须**自动推进并在开始和结束时调用 `opencat-cleanup`；功能实现完成后如条件允许，**必须**调用 `/opencat-auto-test` 自动测试，失败则修复并复测；仅显式带 `worktree` 时才走 worktree，且**严禁**接管 `opencat-work` 的队列调度或最终统一 push。
compatibility: Requires `opencat-cleanup`, `openspec-propose`, `openspec-apply-change`, and `openspec-archive-change` skills to be available. 如果 `opencat-auto-test` 可用且目标可测试，`opencat-task` 应在实现完成后调用它。正常链路下环境检查入口由 `opencat-work` 统一处理，`opencat-task` 不重复执行 `opencat-check`。
---
...
7. **必须**在功能实现完成且规格验证通过后，检查是否满足 `/opencat-auto-test` 的执行条件；若满足，**必须**调用它做自动化测试。凡测试暴露出当前任务范围内可修复的问题，**必须**完成修复并重新测试，**严禁**把可修复失败降级成“跳过测试”。
8. 若执行中发现当前流程未创建的新 TODO、仓库存在预期以外的变更、预期以外的提交、异常任务态 worktree 或无法解释的 Git 状态，**无论如何都严禁暂停**；**必须先将当时仓库中的预期外内容全部提交，不得遗漏，再继续当前流程**。
9. **严禁**删除保留 worktree 目录、改写主干历史、擅自执行队列级最终 `git push`，除非上层流程或用户已明确要求。

### 核心目标

- **必须**完成完整的 OpenSpec purpose / apply / archive / merge 任务链
- **必须**正确执行已选中的 Git 模式：默认分支模式，显式 `worktree` 时才启用 worktree 模式
- 若存在可执行自动化测试条件，**必须**在实现完成后调用 `/opencat-auto-test`，并按“失败即修复、修复后复测”的闭环推进
- **必须**把最终结果安全合并回 `trunk`
...
- 在条件允许时调用 `/opencat-auto-test` 执行自动化测试，并基于结果完成修复与复测
...
开始前固定调用一次 `opencat-cleanup`，把工程收敛到“当前单任务可安全继续”的状态。若 cleanup 后仍存在无法解释的任务态 worktree、脏改动、预期外提交或分支异常，**必须**继续做记录、将当时仓库中的预期外内容全部提交，不得遗漏，并绕行，而不是停下来等待。
...
若验证失败则修复后重试，**严禁**把未通过验证的实现直接推进到自动化测试或提交阶段。

### 8. 自动化测试与修复闭环

在功能实现已完成且 `openspec validate --change "<name>"` 通过后，**必须**立即判定是否满足 `/opencat-auto-test` 的执行条件。

满足执行条件的最低标准：

- 当前环境可用 `/opencat-auto-test`
- 本次 change 存在适合 Web 自动化测试的页面、流程或 URL
- 当前仓库或运行环境能提供可访问的测试入口，例如现成 URL、明确启动方式或可复用的运行实例
- 用户或上游流程没有明确禁止自动化测试

若条件满足：

1. **必须**调用 `/opencat-auto-test`，优先覆盖本次 change 的核心路径与主要回归风险。
2. 若测试失败、阻塞或暴露可修复问题，**必须**先分析根因，并在当前任务范围内完成修复。
3. 修复后**必须**重新执行必要验证，至少再次运行 `openspec validate --change "<name>"`。
4. 然后**必须**再次调用 `/opencat-auto-test`。
5. 重复“测试失败 → 修复 → 验证 → 复测”闭环，直到测试通过，或已明确记录当前环境/测试入口确实不具备继续自动测试的条件。

若条件不满足：

- **必须**明确记录跳过原因，例如缺少技能、无可访问 URL、目标并非 Web 界面、应用未启动且无明确启动方式等
- 记录后允许继续后续阶段，**严禁**凭空臆造测试结果

若自动化测试已执行，则以下规则额外生效：

- **严禁**把当前任务范围内可修复的测试失败，降级成“已知问题，先继续归档/合并”
- 测试失败后产生的修复，仍属于当前 change 的 apply 阶段实现内容，必须纳入同一个 `[apply]` 提交

Apply 提交要求：

- 只暂存 apply 阶段相关文件
- 提交格式：`[apply] <change-name>: <描述>`
- 若执行过 `/opencat-auto-test`，优先在测试通过或已明确记录跳过/阻塞原因后再创建 `[apply]` 提交
...
若执行中出现当前流程未创建的新 TODO、仓库存在预期以外的变更或预期以外的提交：
...
**Stage:** purpose|apply|auto-test|archive|merge|cleanup
...
- SUCCESS: 三个检查点提交全部创建，验证通过；若满足自动化测试条件，则 `/opencat-auto-test` 已执行且对当前任务范围内可修复的问题已完成修复与复测，或已明确记录无法继续自动测试的真实原因；变更合并到主干，中文归档报告生成，`task_branch` 已删除；若为 worktree 模式，关联 slot 已回到配对的 `idle_branch`；若为分支模式，当前工作区已回到 `<base_branch>` 且干净
- FAILURE: 未合并到主干直接结束流程、混入无关更改、分支模式下强行创建 worktree、worktree 模式下 `idle_branch` 缺失或无法恢复、在具备自动化测试条件时跳过 `/opencat-auto-test`、对当前任务范围内可修复的自动化测试失败未修复或未复测、任务完成后工作区仍停在任务分支/主干外/detached 状态、越权接管 `opencat-work` 的队列职责或最终统一 push
...
- 当前未提交改动默认视为允许自动收口的工作流残留；但若判定为预期外变更或预期外提交，**必须先将当时仓库中的预期外内容全部提交，不得遗漏，再继续执行**，不得只处理其中一部分，更不得因此暂停工作
- 请求范围模糊时，先收敛为最小可执行 change；若存在重大设计权衡，优先采用最小改动、最小行为变化方案
- 如果验证结果不完全自信，先做最可能正确的修复并补充验证
- 自动化测试只在当前任务范围内推进；若缺少 `/opencat-auto-test`、无可访问测试入口，或目标本身不适合 Web 自动化测试，**必须**记录真实原因后再继续，**严禁**编造测试结论
