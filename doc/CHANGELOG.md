...
- 将插件版本提升到 `0.2.8`
- 同步更新中英文说明中的稳定执行模型版本标识到 `0.2.8`，并把公开技能列表扩展为 6 个技能，纳入 `opencat-auto-test`
- 将原 `web-playwright-cli-testing` 技能更名为 `opencat-auto-test`，并移除“必须新开独立 Agent 执行测试”的限制
- 更新 `opencat-task`，要求在功能实现完成后如条件允许调用 `/opencat-auto-test` 自动测试；若测试暴露可修复问题，则必须修复并重新测试后再继续后续流程
...
- 将插件版本提升到 `0.2.6`
- 同步更新中英文说明中的稳定执行模型版本标识到 `0.2.6`
- 同步更新中英文说明，对 `opencat-work` 补充“每次只消费一个任务、任务完成后重新读取真实 `TODO.md` / `DONE.md`”的生产者-消费者队列语义
- 调整 `opencat-work` 技能说明，明确已激活章节内新增任务属于正常队列输入，不应默认按异常处理
...
- 将插件版本提升到 `0.2.5`
- 同步更新中英文说明中的稳定执行模型版本标识到 `0.2.5`
- 收紧 `opencat-task` 与 `opencat-work` 对预期外变更和预期外提交的处理要求，明确必须先完整独立提交再继续流程
- 调整 `opencat-work` 顶部说明结构，拆分核心职责与最高工作守则表述
...
- 将插件版本提升到 `0.2.4`
- 同步更新中英文说明中的稳定执行模型版本标识到 `0.2.4`
- `opencat-work` 与 `opencat-task` 不再绑定具体人设或物种，执行者档案仅以 `opencat-agent` 产出为准
- `opencat-agent` 补充边界说明：人设唯一定义处，下游只消费其输出；默认可替换为任意智能体形象
...
- 将插件版本提升到 `0.2.3`
- 同步更新中英文说明中的稳定执行模型版本标识到 `0.2.3`
- 调整 `opencat-task` 与 `opencat-work` 为默认分支模式，只有显式带 `worktree` 时才启用 worktree 工作流
- 同步更新插件描述和中英文说明，明确 worktree 改为显式启用而非默认强制
...
- 将插件版本提升到 `0.2.2`
- 同步更新中英文说明中的稳定执行模型版本标识到 `0.2.2`
- 收紧 `opencat-work` 对“修复”类任务的执行要求，并统一 `DONE.md` 记录格式
...
- 将插件版本提升到 `0.2.0`
- 同步更新中英文说明中的稳定执行模型版本标识到 `0.2.0`
- 作为 `0.2` 版本起点，纳入 `opencat-work` 的强约束化重写，明确 AI 全自动决策与持续推进是最高工作守则
- 明确 `opencat-work` 运行时操作项目内真实 `TODO.md` 与 `DONE.md`，`template/` 下文件仅作只读参考模板
...
- 将插件版本提升到 `0.1.22`
- 同步更新中英文说明中的稳定执行模型版本标识到 `0.1.22`
- 强化 `opencat-work` 的约束文案，明确 AI 全自动决策与持续推进是最高工作守则
- 明确 `opencat-work` 运行时操作项目内真实 `TODO.md` 与 `DONE.md`，`template/` 下文件仅作只读参考模板
...
- 将插件版本提升到 `0.1.20`
- 同步更新中英文说明中的稳定执行模型版本标识到 `0.1.20`
- 更新 `opencat-task` 中归档报告文件名示例，统一为 `<timestamp>-<change-name>.md`
...
- 将插件版本提升到 `0.1.19`
- 同步更新中英文说明中的稳定执行模型版本标识到 `0.1.19`
- 更新 `DONE.md` 约定为追加式归档记录：`[时间] 任务内容-归档文档路径`
- 明确归档文档默认放在 `.claude/docs/opencat/` 下，并同步调整中英文示例
- 同步修正 `opencat-agent` 中关于 `DONE.md` 的旧描述，改为返回归档文档路径供主 Agent 记录
## [0.1.18] - 2026-04-01
